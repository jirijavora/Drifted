...
public struct Particle {
    ...
    /// </summary>
    public bool Active => TimeSinceStart < Lifetime || Lifetime == -1f;

    /// <summary>
    ///     Sets the particle up for first use, restoring defaults
    /// </summary>
    public void Initialize(Vector2 where, float lifetime = 1, float scale = 1, float rotation = 0,
    ...
    /// </summary>
    public void Initialize(Vector2 position, Vector2 velocity, float lifetime = 1, float scale = 1,
        float rotation = 0,
        float angularVelocity = 0, float angularAcceleration = 0, bool active = true) {
        Position = position;
        Velocity = velocity;
    ...
    ///     Sets the particle up for first use
    /// </summary>
    public void Initialize(Vector2 position, Vector2 velocity, Vector2 acceleration, Color color,
        float lifetime = 1,
        float scale = 1, float rotation = 0, float angularVelocity = 0, float angularAcceleration = 0,
        bool active = true) {
        Position = position;
